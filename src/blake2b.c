...
static inline u64 rotr64(u64 w, uint c) { return w >> c | w << (64 - c); }
...
void blake2b_init_param(struct blake2b_state *S, const struct blake2b_param *P) {
	...
	blake2b_init0(S);
	/* IV XOR ParamBlock */
	for (int i = 0; i < 8; ++i) S->h[i] ^= load64(p + sizeof(S->h[i]) * i);
...
void blake2b_init_key(struct blake2b_state *S, u8 outlen, const void *key,
	...
	explicit_bzero(block, BLAKE2B_BLOCKBYTES);
}
...
static void blake2b_compress(struct blake2b_state *S,
	...
	for (int i = 0; i < 16; ++i) m[i] = load64(block + i * sizeof(m[i]));
	...
	for (int i = 0; i < 8; ++i) S->h[i] = S->h[i] ^ v[i] ^ v[i + 8];
}


void blake2b_update(struct blake2b_state *S, const void *pin, ulong inlen) {
	const uchar *in = pin;
	if (inlen > 0) {
		ulong left = S->buflen;
		ulong fill = BLAKE2B_BLOCKBYTES - left;
		if (inlen > fill) {
			S->buflen = 0;
